...
func (r *AuthPolicyEnvoySecurityPolicyReconciler) reconcileSecurityPolicy(ctx context.Context, targetable kuadrantgatewayapi.TargetableNode, kuadrantNamespace string) error {
	logger, _ := logr.FromContext(ctx)
	logger = logger.WithName("reconcileSecurityPolicy")

	esp := envoySecurityPolicy(targetable.Object(), kuadrantNamespace)
		...
		Spec: egv1alpha1.SecurityPolicySpec{
			TargetRef: gatewayapiv1alpha2.PolicyTargetReferenceWithSectionName{
				PolicyTargetReference: gatewayapiv1alpha2.PolicyTargetReference{
					...
					Namespace: ptr.To(gatewayapiv1.Namespace(targetNetworkObject.GetNamespace())),
				},
				SectionName: nil,
			},
